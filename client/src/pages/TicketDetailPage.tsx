...
export default function TicketDetailPage() {
  ...
  const handlePolishReply = async () => {
    ...
    try {
      setIsPolishing(true);
      const res = await axios.post(`${API_URL}/api/ai/polish`, {
        draftReply: replyMessage,
        ticketSubject: ticket?.subject,
        ticketBody: ticket?.body,
        customerName: ticket?.senderName || "Customer",
      });
      if (res.data?.polishedReply) {
        setReplyMessage(res.data.polishedReply);
      }
    } catch (err) {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-gray-400 dark:text-slate-500 font-mono">Ticket #{ticket.id}</span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm space-y-6">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white border-b border-gray-100 dark:border-slate-700 pb-3 flex items-center justify-between">
            ...
            {ticket.replies && ticket.replies.length > 0 ? (
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 rounded-lg space-y-2 border-l-4 border ${
                        isCustomer
                          ? "bg-emerald-50/50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800/60 border-l-emerald-500"
                          : "bg-blue-50/50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800/60 border-l-blue-500"
                      }`}
                    >
                      <div className="flex justify-between items-center text-xs">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-gray-900 dark:text-slate-100">{senderDisplayName}</span>
                          <span
                      ...
                      {reply.bodyHtml ? (
                        <div
                          className="text-sm text-gray-800 dark:text-slate-200 pl-8 prose dark:prose-invert max-w-none"
                          dangerouslySetInnerHTML={{ __html: reply.bodyHtml }}
                        />
                      ) : (
                        <p className="text-sm text-gray-800 dark:text-slate-200 whitespace-pre-wrap pl-8">{reply.message}</p>
                      )}
                    </div>
                  );
                })}
              ...
              <div className="p-4 text-center text-gray-400 dark:text-slate-400 text-sm bg-gray-50 dark:bg-slate-900/50 rounded-lg border border-dashed border-gray-200 dark:border-slate-700">
                No replies yet. Use the form below to post the first reply.
              </div>
            )}

            {/* Submit New Reply Form */}
            <form onSubmit={handleReplySubmit} className="pt-4 border-t border-gray-100 dark:border-slate-700 space-y-4">
              <div className="flex items-center justify-between">
                <label htmlFor="replyInput" className="text-xs text-gray-500 dark:text-slate-400 font-semibold uppercase tracking-wider block">
                  Add a Reply
                </label>
              </div>
              ...
              <textarea
                ...
                className="w-full p-3 border border-gray-300 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 placeholder-gray-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-y"
              ></textarea>
              ...
              <div className="flex justify-end items-center gap-3">
                <button
                  ...
                  disabled={!replyMessage.trim() || isPolishing}
                  className="inline-flex items-center gap-1.5 bg-purple-50 dark:bg-purple-900/40 hover:bg-purple-100 dark:hover:bg-purple-900/60 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800 text-sm font-semibold px-3.5 py-2 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
                >
                  {isPolishing ? (
                    <>
                      <svg className="animate-spin -ml-1 mr-1.5 h-4 w-4 text-purple-700" fill="none" viewBox="0 0 24 24">
